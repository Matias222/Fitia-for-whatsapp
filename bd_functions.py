...
async def insertar_usuario(
    ...
    if len(db_item[1]) == 0:
        new_user = {
            ...
            "edad": edad,
            "objetivo": objetivo,
            "objetivo_confirmado": objetivo_confirmado,
        }
...
async def insertar_user_history(
    id_numero, calorias=0.0, litros=0.0, chat="", temprano=[], tarde=[], noche=[]
):
    ...
    db_item, _ = (
        ...
        .select("*")
        .eq("user_id", id_numero)
        .eq("dia", fecha_actual_str)
        .execute()
    )
    arr_retornar = [1, {}]
...
async def update_calorias(numero_usuario, calorias):
    fecha_actual = datetime.date.today()
    fecha_actual_str = fecha_actual.strftime("%Y-%m-") + str(fecha_actual.day).zfill(2)
    new_calorias = {"calorias": 0}
    try:
        new_calorias = {"calorias": calorias}
